...
function ExerciseEditor({
  ...
  return (
    <div className="border border-white/10 rounded-xl overflow-hidden">
      ...
      {open && (
        <div className="p-3 grid grid-cols-2 gap-3 border-t border-white/10">
          <div>
            ...
            <input type="number" min={1} value={exercise.sets} onChange={(e) => num("sets", e.target.value)}
              className="w-full bg-transparent border border-white/20 rounded-lg px-2 py-1.5 text-white text-sm focus:outline-none focus:border-green-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={exercise.intensity} onChange={(e) => txt("intensity", e.target.value)}
              className="w-full bg-gray-900 border border-white/20 rounded-lg px-2 py-1.5 text-white text-sm focus:outline-none focus:border-green-500">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 flex-shrink-0">
          <div>
            <h2 className="text-white font-bold text-lg">Crear rutina de entrenamiento</h2>
        ...
        <div className="overflow-y-auto flex-1 p-6">
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                <label className="text-gray-300 text-sm font-medium block mb-1">Nombre de la rutina *</label>
                <input
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-gray-300 text-sm font-medium block mb-2">Duración (semanas)</label>
                  <div className="flex gap-2 flex-wrap">
                    {[2, 4, 6, 8, 12].map((w) => (
                      <button key={w} onClick={() => setDurationWeeks(w)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                          durationWeeks === w ? "bg-green-600 border-green-500 text-white" : "border-white/20 text-gray-300 hover:border-white/40"
                        }`}>{w}sem</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="text-gray-300 text-sm font-medium block mb-2">Días de entrenamiento/semana</label>
                  <div className="flex gap-2 flex-wrap">
                    {[2, 3, 4, 5, 6].map((d) => (
                      <button key={d} onClick={() => setDaysPerWeek(d)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                          daysPerWeek === d ? "bg-green-600 border-green-500 text-white" : "border-white/20 text-gray-300 hover:border-white/40"
                        }`}>{d}</button>
                    ))}
                  </div>
                </div>
              ...
              <div>
                <label className="text-gray-300 text-sm font-medium block mb-2">Nivel</label>
                <div className="flex gap-2">
                  {LEVELS.map((l) => (
                    <button key={l} onClick={() => setLevel(l)}
                      className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all ${
                        level === l ? "bg-green-600 border-green-500 text-white" : "border-white/20 text-gray-300 hover:border-white/40"
                      }`}>{l}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="text-gray-300 text-sm font-medium block mb-2">Equipamiento disponible</label>
                <div className="flex flex-wrap gap-2">
                  {EQUIPMENT_OPTIONS.map((eq) => (
                    <button key={eq} onClick={() => setEquipment(eq)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                        equipment === eq ? "bg-green-600 border-green-500 text-white" : "border-white/20 text-gray-300 hover:border-white/40"
                      }`}>{eq}</button>
                  ))}
                </div>
              </div>
          ...
          {step === 2 && day && (
            <div className="space-y-5">
                ...
                <div className="flex gap-1 flex-wrap">
                  {routineDays.map((d, i) => (
                    <button key={i} onClick={() => setActiveDay(i)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                        activeDay === i ? "bg-green-600 border-green-500 text-white" : "border-white/20 text-gray-400 hover:border-white/40"
                      }`}>{d.day_name}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <textarea rows={2} value={day.warmup || ""} onChange={(e) => updateDay(activeDay, { ...day, warmup: e.target.value })}
                    className="w-full bg-white/5 border border-white/20 rounded-lg px-3 py-2 text-white text-xs resize-none focus:outline-none focus:border-green-500" />
                </div>
                <div>
              ...
              <div className="space-y-2">
                <p className="text-gray-300 text-sm font-semibold">Ejercicios <span className="text-gray-500 font-normal text-xs">— hacé clic en un ejercicio para editar sus detalles</span></p>
