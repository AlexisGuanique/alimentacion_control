...
import { MealPlan, MealPlanContent, MealPlanFood, MealPlanMeal, updateMealPlan } from "@/lib/api";
...
function FoodEditor({
    ...
    return (
      <div>
        ...
        <input
          ...
          className="w-full bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-gray-900 text-sm focus:outline-none focus:border-green-500"
        />
      </div>
    );
  }
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      ...
      >
        <span className="text-gray-800 text-sm font-medium truncate">{food.name}</span>
      ...
      {open && (
        <div className="p-4 grid grid-cols-2 gap-3 border-t border-gray-100">
          ...
          <div className="col-span-2">
            <label className="text-gray-500 text-xs block mb-1">Preparación</label>
            <textarea
              rows={2}
              value={food.preparation || ""}
              onChange={(e) => onChange({ ...food, preparation: e.target.value })}
              className="w-full bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-gray-900 text-sm resize-none focus:outline-none focus:border-green-500"
            />
          </div>
        </div>
      )}
...
export default function EditMealPlanModal({ plan, onClose, onUpdated }: Props) {
    ...
    const days = content.days.map((d, di) => {
      ...
      const meals = d.meals.map((m, mi) => {
        ...
        return {
          ...
          total_protein_g: foods.reduce((s, f) => s + f.protein_g, 0),
          total_carbs_g: foods.reduce((s, f) => s + f.carbs_g, 0),
          total_fat_g: foods.reduce((s, f) => s + f.fat_g, 0),
        };
      });
      return { ...d, meals, total_calories: meals.reduce((s, m) => s + m.total_calories, 0) };
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-2xl shadow-xl flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <h2 className="text-gray-900 font-bold text-lg">Editar Plan de Comidas</h2>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none focus:border-green-500"
              />
            </div>
            <div>
              <label className="text-gray-700 text-sm font-medium block mb-1">Descripción</label>
              <textarea
                rows={2}
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm resize-none focus:outline-none focus:border-green-500"
              />
            </div>
          </div>
          ...
          {/* Day tabs */}
          <div>
            <p className="text-gray-500 text-xs font-medium uppercase tracking-wider mb-2">Días del plan</p>
          ...
          {day && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-gray-900 font-semibold">{day.day_name}</h3>
                <span className="text-green-700 text-sm font-medium">
                  {Math.round(day.total_calories)} kcal
                </span>
              </div>
              {day.meals.map((meal, mealIdx) => (
                <div key={mealIdx} className="bg-gray-50 rounded-xl p-4 space-y-2 border border-gray-100">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <span className="text-gray-800 font-semibold text-sm">{meal.meal_type}</span>
                      {meal.time_suggestion && (
                        <span className="text-gray-400 text-xs ml-2">{meal.time_suggestion}</span>
                      )}
                    </div>
                    <span className="text-orange-600 text-sm">{Math.round(meal.total_calories)} kcal</span>
                  </div>
